...
if (isset($_POST["cod_usu"])){
    ...
    $SQL_IN="INSERT INTO EJERCITO VALUES ($cod_ejer, '$nom', $Raza, $Fac, $cod_usu)";
    echo "$cod_usu";
}
else{
    $SQL_IN="INSERT INTO EJERCITO VALUES ($cod_ejer, '$nom', $Raza, $Fac, null)";
}
...
<form action='ejer_insert.php' method='POST'>
<input type="hidden" name="cod_ejer" value="<?php echo $cod_ejer;?>">
<?php
    $SQL="SELECT * FROM UNIDAD WHERE Raza=$cod_raza";
...
<?php
    $SQL="SELECT * FROM VEHICULOS WHERE RAZA=$cod_raza";
    $RS=mysqli_query($con, $SQL);
    while($row=mysqli_fetch_assoc($RS)){
    $cod_ve=$row["cod"];
    $nom_ve=$row["Nombre"];
...
<?php
    $SQL="SELECT * FROM ANDADORES WHERE RAZA=$cod_raza";
    $RS=mysqli_query($con, $SQL);
    while($row=mysqli_fetch_assoc($RS)){
    $cod_an=$row["cod"];
    $nom_an=$row["Nombre"];
